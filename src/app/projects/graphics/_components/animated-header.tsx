...
export function AnimatedHeader({ className }: { className?: string }) {
  return (
    <motion.section
      className={cn(
        "relative mx-auto flex max-w-4xl min-w-3xl flex-col items-center justify-center rounded-xl border border-white/10 bg-black/80 px-8 py-10 shadow-xl backdrop-blur-md",
        className,
      )}
      initial="hidden"
      animate="show"
        ...
        <motion.div variants={item} className="w-full max-w-2xl">
          ...
          >
            {projectItems.map(({ id, title, description, icon, link }) => (
              ...
              >
                <AccordionTrigger className="group flex w-full items-center justify-between py-4 text-left text-white transition hover:text-blue-400 focus:text-blue-400">
